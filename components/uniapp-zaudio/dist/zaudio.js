...
class ZAudio extends util_1.EventBus {
    ...
    operation() {
        return __awaiter(this, void 0, void 0, function* () {
            const { duration, current, duration_value, current_value, src, id } = this.playinfo;
            const { src: renderSrc, title: renderTitle, singer: renderSinger, coverImgUrl: renderCoverImgUrl, id: renderId } = this.renderinfo;
    ...
    setRender(data) {
        ...
        if (typeof data === "number" || typeof data === "string") {
            ...
            this.renderinfo = {
				id: this.audiolist[this.renderIndex].id,
            ...
            let renderIndex = this.audiolist.findIndex((i) => i.id == data.id);
            if (renderIndex >= 0) {
    ...
    //当前索引
    get playIndex() {
        let index = this.audiolist.findIndex((i) => i.id == this.playinfo.id);
        return index <= 0 ? 0 : index;
    }
    //渲染与播放是否一致
    get renderIsPlay() {
        return this.renderinfo.id == this.playinfo.id;
    }
